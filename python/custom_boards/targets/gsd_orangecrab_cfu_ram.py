...
# cpu.CPUS.update({"vexriscv_cfu_ram": VexRiscvCFURAM})
    ...
    def __init__(
        ...
        **kwargs,
        ...
        platform = gsd_orangecrab.Platform(
            revision=revision, device=device, toolchain=toolchain
        )

        # CRG --------------------------------------------------------------------------------------
        crg_cls = (
            _CRGSDRAM
            if kwargs.get("integrated_main_ram_size", 0) == 0
            else _CRG
        )
        self.submodules.crg = crg_cls(
            platform, sys_clk_freq, with_usb_pll=True
        )
        ...
        # kwargs["cpu_type"] = "vexriscv_cfu_ram"
        SoCCore.__init__(
            self,
            platform,
            sys_clk_freq,
...
def main():
    ...
    target_group.add_argument(
        "--load", action="store_true", help="Load bitstream."
    )
    target_group.add_argument(
        "--toolchain",
        default="trellis",
    ...
    target_group.add_argument(
        "--sys-clk-freq", default=48e6, help="System clock frequency."
    )
    target_group.add_argument(
        "--revision", default="0.2", help="Board Revision (0.1 or 0.2)."
    )
    target_group.add_argument(
        "--device", default="25F", help="ECP5 device (25F, 45F or 85F)."
    )
    target_group.add_argument(
        "--sdram-device",
        default="MT41K64M16",
    ...
    builder_kargs = (
        trellis_argdict(args) if args.toolchain == "trellis" else {}
    )
    if args.build:
