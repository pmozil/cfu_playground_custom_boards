...
from litedram.phy.model import (SDRAMPHYModel, get_sdram_phy_settings,
...
from litex.build.sim.verilator import (verilator_build_argdict,
        ...
        **kwargs
        ...
        if with_ethernet or with_etherbone:
            if ethernet_phy_model == "sim":
                ...
                raise ValueError(
                    "Unknown Ethernet PHY model:", ethernet_phy_model
                )
        ...
        if with_ethernet and with_etherbone:
            ...
            self.submodules.icmp = LiteEthICMP(
                self.ip, etherbone_ip_address, dw=8
            )
        ...
        # Ethernet ---------------------------------------------------------------------------------
            ...
            self.add_wb_slave(
                self.mem_regions["ethmac"].origin,
                ethmac.bus,
                ethmac_region_size,
            )
            if self.irq.enabled:
        ...
        if with_spi_flash:
            ...
            self.submodules.spiflash_phy = LiteSPIPHYModel(
                spiflash_module, init=spi_flash_init
            )
            self.add_spi_flash(
        ...
        if with_gpio:
            self.submodules.gpio = GPIOTristate(
                platform.request("gpio"), with_irq=True
            )
...
def generate_gtkw_savefile(builder, vns, trace_fst):
    ...
    with gtkw.GTKWSave(vns, savefile=savefile, dumpfile=dumpfile) as save:
        ...
        if hasattr(soc, "sdrphy"):
            ...
            save.add(
                soc.sdrphy.dfi,
                mappers=[gtkw.dfi_sorter(), gtkw.dfi_in_phase_colorer()],
            )
...
def sim_args(parser):
    ...
    parser.add_argument(
        "--rom-init", default=None, help="ROM init file (.bin or .json)."
    )
    parser.add_argument(
        "--ram-init", default=None, help="RAM init file (.bin or .json)."
    )
    parser.add_argument(
        "--with-sdram", action="store_true", help="Enable SDRAM support."
    )
    parser.add_argument(
        "--sdram-module", default="MT48LC16M16", help="Select SDRAM chip."
    )
    parser.add_argument(
        "--sdram-data-width", default=32, help="Set SDRAM chip data width."
    )
    parser.add_argument(
        "--sdram-init", default=None, help="SDRAM init file (.bin or .json)."
    )
    parser.add_argument(
        "--sdram-from-spd-dump",
        default=None,
    ...
    parser.add_argument(
        "--with-i2c", action="store_true", help="Enable I2C support."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--spi_flash-init", default=None, help="SPI Flash init file."
    )
    parser.add_argument(
        "--with-gpio",
        action="store_true",
        help="Enable Tristate GPIO (32 pins).",
    )
    parser.add_argument(
        "--sim-debug",
        action="store_true",
...
def main():
    ...
    if args.integrated_main_ram_size:
        ...
        if args.sdram_init is not None:
            soc_kwargs["sdram_init"] = get_mem_data(
                args.sdram_init,
                endianness=cpu.endianness,
                offset=ram_boot_offset,
            )
    ...
    if args.with_ethernet or args.with_etherbone:
        if args.ethernet_phy_model == "sim":
            ...
            raise ValueError(
                "Unknown Ethernet PHY model: " + args.ethernet_phy_model
            )
    ...
    soc = SimSoC(
        ...
        spi_flash_init=None
        if args.spi_flash_init is None
        else get_mem_data(args.spi_flash_init, endianness="big"),
        **soc_kwargs,
    )
