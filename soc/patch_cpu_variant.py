...
# from vexriscv_custom_cpu import core as vexriscv_core
...
def build_cpu_variant_if_needed(variant):
    if (
        variant in vexriscv_core.CPU_VARIANTS
        or variant in serv_core.CPU_VARIANTS
    ):
    ...
    if params[0] != "generate":
        print(
            "ERROR: Need generate keyword to begin on demand Vexriscv build."
        )
        exit()
