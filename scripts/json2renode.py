...
def generate_sysbus_registration(
    descriptor, skip_braces=False, region=None, skip_size=False
):
    ...
    def generate_registration_entry(address, size=None, name=None):
        if name:
            if not size:
                raise Exception(
                    "Size must be provided when registering non-default region"
                )
            return 'sysbus new Bus.BusMultiRegistration {{ address: {}; size: {}; region: "{}" }}'.format(
    ...
    size = (
        descriptor["size"] if "size" in descriptor and not skip_size else None
    )
    ...
    if "shadowed_address" in descriptor:
        result = "{}; {}".format(
            ...
            generate_registration_entry(
                descriptor["shadowed_address"], size, region
            ),
        )
    else:
...
""".format(
        "_CSR32" if csr["constants"]["config_csr_data_width"] == 32 else "",
        generate_sysbus_registration(peripheral, skip_braces=True),
        generate_sysbus_registration(buf, skip_braces=True, region="buffer"),
        generate_sysbus_registration(phy, skip_braces=True, region="phy"),
    )

    interrupt_name = "{}_interrupt".format(name)
    if interrupt_name in csr["constants"]:
        result += "    -> cpu@{}\n".format(csr["constants"][interrupt_name])
...
was {}.
""".format(
            hex(region_descriptor["original_address"])
        )
...
was {} bytes.
""".format(
            hex(region_descriptor["original_size"])
        )
...
def get_cpu_type(csr):
    ...
    config_cpu_type = next(
        (
            k
            for k in csr["constants"].keys()
            if k.startswith("config_cpu_type_")
        ),
        None,
    )
    if config_cpu_type:
    ...
    config_cpu_variant = next(
        (
            k
            for k in csr["constants"].keys()
            if k.startswith("config_cpu_variant_")
        ),
        None,
    )
...
def generate_cpu(csr, time_provider):
    ...
    if (
        kind == "vexriscv"
        or kind == "vexriscv_smp"
        or kind == "vexriscv_cfu_ram"
    ):
        result = """
...
def generate_peripheral(csr, name, **kwargs):
    ...
    model = kwargs["model"]
    if (
        csr["constants"]["config_csr_data_width"] == 32
        and "model_CSR32" in kwargs
    ):
    ...
    for constant, val in peripheral["constants"].items():
        if (
            "ignored_constants" not in kwargs
            or constant not in kwargs["ignored_constants"]
        ):
...
def generate_cas(csr, name, **kwargs):
    result = generate_peripheral(
        csr,
        name,
        model="GPIOPort.LiteX_ControlAndStatus",
        ignored_constants=["leds_count", "switches_count", "buttons_count"],
    )
...
""".format(
        "_CSR32" if csr["constants"]["config_csr_data_width"] == 32 else "",
        generate_sysbus_registration(peripheral, skip_braces=True),
        generate_sysbus_registration(core, skip_braces=True, region="core"),
        generate_sysbus_registration(
            reader, skip_braces=True, region="reader"
        ),
        generate_sysbus_registration(
            writer, skip_braces=True, region="writer"
        ),
    )
...
clint: IRQControllers.CoreLevelInterruptor @ {}
    ...
    [0, 1] -> cpu@[101, 100]
""".format(
        generate_sysbus_registration(clint, skip_braces=True, skip_size=True),
        frequency,
    )
...
""".format(
        generate_sysbus_registration(plic, skip_braces=True, skip_size=True)
    )
...
def get_clock_frequency(csr):
    ...
    # has different names
    return csr["constants"][
        "config_clock_frequency"
        if "config_clock_frequency" in csr["constants"]
        else "system_clock_frequency"
    ]
...
peripherals_handlers = {
    ...
    "timer0": {
        ...
        "properties": {"frequency": lambda c: get_clock_frequency(c)},
    },
    "ethmac": {
    ...
    "ddrphy": {"handler": generate_silencer},
    "sdram": {"handler": generate_silencer},
    "spiflash": {"handler": generate_spiflash},
    "spi": {
        "handler": generate_peripheral,
        "model": "SPI.LiteX_SPI",
        "ignored_constants": [
            "interrupt"
        ],  # model in Renode currently doesn't support interrupts
    },
    "ctrl": {
        "handler": generate_peripheral,
...
def generate_repl(csr, etherbone_peripherals, autoalign):
    ...
    for mem_region in filter_memory_regions(
        memories, alignment=0x1000, autoalign=autoalign
    ):
        result += generate_memory_region(mem_region)

    time_provider = None
    if "clint" in csr["memories"]:
        result += generate_clint(
            csr["memories"]["clint"],
    ...
    for name, address in csr["csr_bases"].items():
        if name not in peripherals_handlers:
            print(
                "Skipping unsupported peripheral `{}` at {}".format(
                    name, hex(address)
                )
            )
            continue
...
def filter_memory_regions(raw_regions, alignment=None, autoalign=[]):
    ...
    for r in raw_regions:
        ...
        if alignment is not None:
            ...
            if address_mismatch != 0:
                if r["name"] in autoalign:
                    ...
                    print(
                        "Re-aligning `{}` memory region base address from {} to {} due to limitations in Renode".format(
                            r["name"],
                            hex(r["original_address"]),
                            hex(r["base"]),
                        )
                    )
                else:
                    print(
                        "Error: `{}` memory region base address ({}) is not aligned to {}. This configuration cannot be currently simulated in Renode".format(
                            r["name"], hex(r["size"]), hex(alignment)
                        )
                    )
                    sys.exit(1)
            ...
            if size_mismatch != 0:
                if r["name"] in autoalign:
                    ...
                    print(
                        "Extending `{}` memory region size from {} to {} due to limitations in Renode".format(
                            r["name"], hex(r["original_size"]), hex(r["size"])
                        )
                    )
                else:
                    print(
                        "Error: `{}` memory region size ({}) is not aligned to {}. This configuration cannot be currently simulated in Renode".format(
                            r["name"], hex(r["size"]), hex(alignment)
                        )
                    )
                    sys.exit(1)
        ...
        if (
            ...
            and r["base"] >= previous_region["base"]
            and r["base"] + r["size"]
            <= previous_region["base"] + previous_region["size"]
        ):
            print(
                "Skipping `{}` since it is included in `{}`".format(
                    r["name"], previous_region["name"]
                )
            )
            continue
        ...
        if (
            previous_region is not None
            and r["base"] < previous_region["base"] + previous_region["size"]
            and r["base"] + r["size"]
            > previous_region["base"] + previous_region["size"]
        ):
            print(
                "Error: `{}` overlaps with `{}` - this configuration cannot be currently simulated in Renode".format(
                    r["name"], previous_region["name"]
                )
            )
            sys.exit(1)
...
""".format(
    ...
    rom_base = (
        csr["memories"]["rom"]["base"] if "rom" in csr["memories"] else None
    )
...
cpu PC {}
""".format(
            args.bios_binary, rom_base, rom_base
        )
...
server StartTFTP {}
""".format(
            args.tftp_ip, args.tftp_port
        )

        for name, path in tftp_binaries.items():
            result += """
server.tftp ServeFile @{} "{}" """.format(
                path, name
            )
...
""".format(
        ...
        if "flash_boot_address" not in csr["constants"]:
            ...
            for offset in flash_binaries:
                ...
                flash_boot_address = (
                    int(csr["constants"]["flash_boot_address"], 0) + offset
                )
                ...
                result += "sysbus WriteDoubleWord {} {}\n".format(
                    hex(flash_boot_address), hex(len(firmware_data))
                )
                result += "sysbus WriteDoubleWord {} {}\n".format(
                    hex(flash_boot_address + 4), hex(crc32)
                )
                result += "sysbus LoadBinary @{} {}\n".format(
                    path, hex(flash_boot_address + 8)
                )
...
def parse_flash_binaries(csr, args):
    ...
    if args.flash_binaries_args:
        for entry in args.flash_binaries_args:
            ...
            if separator == "":
                print(
                    "Flash binary '{}' is in a wrong format. It should be 'path:offset'".format(
                        entry
                    )
                )
                sys.exit(1)
            ...
            try:
                ...
                if offset_or_label in csr["constants"]:
                    ...
                    print(
                        "Offset is in a wrong format. It should be either a number or one of the constants from the configuration file:"
                    )
                    print(
                        "\n".join(
                            "\t{}".format(c) for c in csr["constants"].keys()
                        )
                    )
                    sys.exit(1)
...
def check_tftp_binaries(args):
    ...
    for entry in args.tftp_binaries_args:
        ...
        if name in tftp_binaries:
            print(
                "File with name {} specified more than one - please check your configuration.".format(
                    name
                )
            )
            sys.exit(1)
...
def check_etherbone_peripherals(peripherals):
    ...
    for p in peripherals:
        ...
        if separator == "":
            print(
                "Etherbone peripheral `{}` is in a wrong format. It should be in 'name:port'".format(
                    p
                )
            )
            sys.exit(1)
        ...
        if name not in peripherals_handlers:
            print(
                "Unsupported peripheral '{}'. Available ones:\n".format(name)
            )
            print(
                "\n".join(
                    "\t{}".format(c) for c in peripherals_handlers.keys()
                )
            )
            sys.exit(1)
...
def parse_args():
    ...
    parser.add_argument(
        "--bios-binary", action="store", help="Path to the BIOS binary"
    )
    parser.add_argument(
        "--firmware-binary",
        action="store",
...
def main():
    ...
    etherbone_peripherals = check_etherbone_peripherals(
    ...
    if args.repl:
        print_or_save(
            args.repl,
            generate_repl(
                csr, etherbone_peripherals, args.autoalign_memor_regions
            ),
        )
    ...
    if args.resc:
        if not args.repl:
            ...
            print_or_save(
                args.resc,
                generate_resc(csr, args, flash_binaries, tftp_binaries),
            )
